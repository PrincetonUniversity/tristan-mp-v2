...
parser.add_argument(
    "-c", action="store_true", default=False, help="only test compilation."
)
parser.add_argument(
    "-r", action="store_true", default=False, help="only run simulations."
)
parser.add_argument(
    "-v", action="store_true", default=False, help="verbose mode (full output)."
)
parser.add_argument(
    "-d", "--diag", type=int, default=None, help="diagnostic mode [choose test #]."
)
parser.add_argument(
    "-t",
    "--test",
    type=str,
    default=",".join(map(str, list(range(15)))),
    help="tests to run.",
)
...
if options.cluster == "perseus":
    ...
    modules = ["intel-parallel-studio/2020", "hdf5p/1.10.7", "openmpi/gcc/64/4.1.5a1"]
...
class PlasmaOsc(Simulation):
    ...
    def diag(self):
        ...
        ax.plot(
            steps
            * self.params["output"]["interval"]
            / (2 * np.pi * self.params["plasma"]["c_omp"] / 0.45),
            exs,
        )
        ax.set_xlabel(r"$t\omega_{\rm p0}$")
        ax.set_ylabel(r"$E_x$")
        ...
        ax.text(
            ...
            by $t\omega_{{\rm p 0}}={{{}}}: \Delta E/E={{{}}}%$
            """.format(
                int(hist["time"][-1] * 0.45 / self.params["plasma"]["c_omp"]),
                int(hist["% dEtot"][-1] * 10000) / 10000,
            ),
        )
        plt.tight_layout()
...
class Merging(Simulation):
    ...
    def diag(self):
        ...
        def getMomenta(prtls):
            ...
            mx = np.sum(prtls["1"]["u"] * prtls["1"]["wei"]) + np.sum(
                prtls["2"]["u"] * prtls["2"]["wei"]
            )
            my = np.sum(prtls["1"]["v"] * prtls["1"]["wei"]) + np.sum(
                prtls["2"]["v"] * prtls["2"]["wei"]
            )
            mz = np.sum(prtls["1"]["w"] * prtls["1"]["wei"]) + np.sum(
                prtls["2"]["w"] * prtls["2"]["wei"]
            )
            return (en, mx, my, mz)
        ...
        def template(prtls):
            ...
            return (
                r"npart: {} ($\times$2)".format(len(prtls["1"]["x"]))
                + "\ntotal energy [err%]: {:.3f} [{:.4f}%]".format(
                    energy1, np.abs((energy1 - energy0) * 100 / energy0)
                )
                + "\nmomX [err%]: {:.3f} [{:.4f}%]".format(
                    momx1, np.abs((momx1 - momx0) * 100 / momx0)
                )
                + "\nmomY [err%]: {:.3f} [{:.4f}%]".format(
                    momy1, np.abs((momy1 - momy0) * 100 / momy0)
                )
        ...
        ax.grid(False)
        lines1 = [
            [(x, y) for y in (0, self.params["grid"]["mx0"])] for x in major_ticks
        ...
        lines2 = [
            [(x, y) for y in (0, self.params["grid"]["mx0"])] for x in minor_ticks
        ...
        im = ax.imshow(
            np.zeros((self.params["grid"]["mx0"], self.params["grid"]["my0"])),
            origin="lower",
            extent=(0, self.params["grid"]["mx0"], 0, self.params["grid"]["my0"]),
            cmap="seismic",
            vmin=-divE_max,
            vmax=divE_max,
            zorder=0,
        ...
        ttl = ax.text(0.1, 0.1, "", transform=ax.transAxes, va="center")
        diag = ax.text(
            2,
            47,
            "",
        ...
        def init():
            ...
            ax.set_ylabel(r"$y$")
            ax.set_title(self.jobid)
            prtls = isolde.getParticles(self.path + "/output/prtl/prtl.tot.%05d" % 0)
            flds = isolde.getFields(self.path + "/output/flds/flds.tot.%05d" % 0)
        ...
        def animate(i):
            prtls = isolde.getParticles(self.path + "/output/prtl/prtl.tot.%05d" % i)
            flds = isolde.getFields(self.path + "/output/flds/flds.tot.%05d" % i)
            sc1.set_offsets(np.array([prtls["1"]["x"], prtls["1"]["y"]]).T)
            sc2.set_offsets(np.array([prtls["2"]["x"], prtls["2"]["y"]]).T)
            im.set_data(flds["divE"][0])
    ...
    def diag(self):
        ...
        def draw(i):
            ...
            for ax, coords, avr in zip([ax1, ax2, ax3], ["xz", "xy", "yz"], [1, 0, 2]):
                ...
                im = ax.imshow(
                    np.zeros((sizes[s1], sizes[s2])),
                    vmin=0,
                    vmax=300,
                    origin="lower",
                    extent=(0, sizes[s1], 0, sizes[s2]),
                    cmap="hot",
                )
...
if options.diag:
    ...
    with open(testdir_full + "/test.log", "w+") as testlog:
        ...
        for ii, simulation in enumerate(simulations):
            ...
            testlog.write(
                ("TEST_#{}_".format(ii + 1) + simulation.jobid).ljust(50, ".") + "\n"
            )
            # configure
            ...
            config_command += (
                " --{}=".format(simulation.userfile[:4]) + simulation.userfile
            )
            if not options.r:
                ...
                if (
                    not os.path.isfile(codedir + "/bin/" + simulation.exe)
                ) and not options.dry:
                    raise RuntimeError("Something went wrong")
                testlog.write("compilation".ljust(46, ".") + "[OK]\n")
                print("Compilation of `{}` done.".format(simulation.jobid))
                # move executable
                callCommand(f"mv {codedir}/bin/{simulation.exe} {simulation.path}")
                # clean
                callCommand("make clean" + suffix)
                # write input
        ...
        if (not options.c) or (options.r):
            for ii, simulation in enumerate(simulations):
                if not (ii + 1 in tests):
                    continue
                callCommand(f"cd {simulation.path} && sbatch {simulation.submit_full}")
                testlog.write(
                    ("`{}`".format(simulation.jobid)).ljust(41, ".") + "submitted\n"
                )
