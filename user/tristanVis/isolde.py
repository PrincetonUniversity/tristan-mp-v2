    ...
    with h5py.File(fname, "r") as file:
        ...
        for s in range(nspec):
            ...
            for i in range(nvars):
                if not (variables[i] + "_" + str(s + 1)) in file:
                    continue
                (data[str(s + 1)])[variables[i]] = file[
                    variables[i] + "_" + str(s + 1)
                ][:]
    return data
...
def getSlice(output, proj, step):
    ...
    return getFields(
        output + "slice" + proj.upper() + "=" + "%05d" % int(shift) + ".%05d" % step
    )


def convertToXarray(
    fields,
    coordinateTransformation={"x": lambda f: f, "y": lambda f: f, "z": lambda f: f},
    additionalVariables={},
    mask=None,
):
    import xarray as xr
    ...
    if dimension == 1:
        xr_axes = np.array(list("xyz"))[
            [(np.min(fields[p + p]) != np.max(fields[p + p])) for p in list("xyz")]
        ]
        if len(xr_axes) != 1:
        ...
        xr_data.coords[x1] = (
            (x1),
            coordinateTransformation[x1](fields[x1 * 2][:, 0, 0]),
        )
        xr_data.coords[x2] = (
            (x2),
            coordinateTransformation[x2](fields[x2 * 2][0, :, 0]),
        )
        xr_data.coords[x3] = (
            (x3),
            coordinateTransformation[x3](fields[x3 * 2][0, 0, :]),
        )
    for k in fields.keys():
...
class Spectra:
    ...
    def findSpecname(self, s, onlyGCA, onlyBoris):
        if not self.__gca and (onlyGCA or onlyBoris):
            ...
            raise ValueError(
                "At least one of `onlyGCA` and `onlyBoris` have to be `False`."
            )
    ...
    def initialize(self, raw):
        ...
        if self.__radiation:
            ...
            radspecies = [
                int(re.findall("^nr(\d+)", key)[0])
                for key in keylist
                if re.match("^nr\d+", key)
            ]
            ...
            if routine != "" and routine[0] != "[":
                ...
                if len(nums) < 3:
                    ...
                    if not writing_particles:
                        ...
                        data[routine]["min"] = np.append(
                            data[routine]["min"], [nums[1]]
                        )
                        data[routine]["max"] = np.append(
                            data[routine]["max"], [nums[2]]
                        )
                    else:
                        data[routine]["average"] = np.append(
                            data[routine]["average"], [nums[0]]
                        )
                        data[routine]["min"] = np.append(
                            data[routine]["min"], [nums[1]]
                        )
                        ...
                        data[routine]["total"] = np.append(
                            data[routine]["total"], [nums[3]]
                        )
    ...
    with open(fname, "r") as f:
        ...
        while block:
            ...
            block_values = np.array(
                re.findall("-?\ *[0-9]+\.?[0-9]*(?:[Ee]\ *[-|+]?\ *[0-9]+)?", block)
            )
            if len(block) == 0:
                break
            block_values = np.array(list(map(float, block_values)))
        ...
        for k, group in groupby(f, key=make_grouper()):
            ...
            for b in fasta_section:
                ...
                if ":" in b:
                    ...
                    array = np.array([float(v) for v in b.split(",") if is_float(v)])
